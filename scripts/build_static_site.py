...
def _header(active: str) -> str:
    """Shared top nav. `active` is one of 'home', 'summary', 'runs', 'parameters', 'transitions'."""
    items = [
...
_RUNS_STYLE = _STYLE_COMMON + r"""
  ...
  .blurb { color:#555; font-size:12.5px; margin:0 0 10px; max-width:80ch; line-height:1.5; }
  .rid { font-family:ui-monospace,Menlo,monospace; font-size:11px; color:#888; }
  .verdict { display:inline-block; padding:1px 8px; border-radius:10px; font-size:11px; font-weight:600; }
  .verdict.good { background:#d4f4d4; color:#0a6c2c; } .verdict.bad { background:#fde2df; color:#b32020; }
  .verdict.neutral { background:#eee; color:#555; }
  .agree { display:inline-block; height:6px; border-radius:2px; background:#5b3aa6; vertical-align:middle; margin-left:6px; opacity:.75; }
"""

_RUNS_HTML = r"""<!doctype html>
<html><head><meta charset="utf-8"><title>Runs &mdash; AI Impact Meta-Review</title>
<style>__STYLE__</style></head><body>
__HEADER__
<main>
  <p class="intro" id="intro">
    Published analysis runs. Click a row to open its results, or see the
    <a href="summary.html">Summary</a> for charts and findings across the whole set.
    See <a href="parameters.html">Parameters</a> for what the knobs do.
  </p>
  <div id="groups"></div>
</main>
<script>
...
function table(head, body){ return `<table><thead><tr>${head.map(h=>`<th${h[1]?' class="num"':''}>${h[0]}</th>`).join('')}</tr></thead><tbody>${body}</tbody></table>`; }

function renderGroup(g, runs){
  let html = `<h2>${escapeHtml(g.title)}</h2><p class="blurb">${escapeHtml(g.blurb||'')}</p>`;
  if (g.id === 'validation') {
    html += table([['Run'],['Metric'],['Setting'],['Folds',1],['Kendall τ-b',1],['95% CI',1],['Spearman ρ',1],['MAE',1],['Verdict']],
      runs.map(r => { const s=r.loo_summary||{}, rk=(s.rank||{}).overall||{}, o=s.overall||{};
        return row(r, [`<td class="rid">${escapeHtml(r.run_id)}</td>`, `<td>${pill(r.params.metric)}</td>`,
    ...
    html += table([['Run'],['Metric'],['Aggregation'],['β',1],['Baseline'],['Obs occ/act',1],['Kept occ/act',1],['Occ ρ vs occupation run',1]],
      runs.map(r => { const p=r.params||{}, a=(r.agreement||{}).occupation;
        return row(r, [`<td class="rid">${escapeHtml(r.run_id)}</td>`, `<td>${pill(p.metric)}</td>`, `<td>${escapeHtml(lvl(r))}</td>`,
...
fetch('data/runs/index.json').then(r=>r.json()).then(d=>{
  ...
  if (!d.runs.length) { el.innerHTML='<p style="color:#888;">No runs published.</p>'; return; }
...
# ---------- summary.html (curated-run overview; same design as the review artifact) ----------

_SUMMARY_HTML = r"""<!doctype html>
<html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>Run summary &mdash; AI Impact Meta-Review</title>
...
<style>__STYLE_COMMON__
:root{
  --bg:#f5f6f8; --surface:#ffffff; --ink:#1b2230; --ink-2:#4d5566; --muted:#7a8293; --rule:#dde1e8; --grid:#eceff3;
  --speed:#2a78d6; --quality:#eb6834;
  --good:#1a7f4b; --good-bg:#e3f3ea; --bad:#b8322a; --bad-bg:#fbe6e3; --neutral-bg:#eceef2;
}
@media (prefers-color-scheme: dark){
  :root:not([data-theme="light"]){
    color-scheme: dark;
    --bg:#12151b; --surface:#1a1e26; --ink:#e8ecf2; --ink-2:#b7beca; --muted:#8a92a1; --rule:#2c323d; --grid:#242a33;
    --speed:#3987e5; --quality:#d95926;
    --good:#5cc48c; --good-bg:#15321f; --bad:#f07a6e; --bad-bg:#3a1c19; --neutral-bg:#262b34;
  }
}
...
body{background:var(--bg);color:var(--ink);font:14px/1.55 "IBM Plex Sans",system-ui,-apple-system,sans-serif}
.wrap{max-width:1040px;margin:0 auto;padding-inline:16px;padding-block:28px 60px;display:flex;flex-direction:column;gap:34px}
.wrap h1,.wrap h2{font-family:"Newsreader",Georgia,serif;font-weight:600;text-wrap:balance;margin:0;letter-spacing:-.01em}
.wrap h1{font-size:34px;line-height:1.1}
.wrap h2{font-size:23px}
.wrap h3{font-size:12px;text-transform:uppercase;letter-spacing:.07em;color:var(--muted);margin:0;font-weight:600}
.wrap p{margin:0;max-width:68ch;color:var(--ink-2)}
.wrap a{color:var(--speed)}
...
.facts{display:flex;flex-wrap:wrap;gap:8px 26px;margin-top:6px;font-size:13px;color:var(--ink-2)}
.facts b{color:var(--ink);font-family:"IBM Plex Mono",monospace;font-weight:500}
section{display:flex;flex-direction:column;gap:14px}
.findings{display:grid;grid-template-columns:repeat(auto-fit,minmax(230px,1fr));gap:12px}
.finding{background:var(--surface);border:1px solid var(--rule);border-radius:8px;padding:14px 16px;display:flex;flex-direction:column;gap:8px}
...
.wrap table{border-collapse:collapse;width:100%;font-size:13px}
.wrap th{font-size:11px;text-transform:uppercase;letter-spacing:.06em;color:var(--muted);font-weight:600;text-align:left;padding:6px 10px;border-bottom:1px solid var(--rule);white-space:nowrap}
.wrap td{padding:7px 10px;border-bottom:1px solid var(--grid);vertical-align:top}
.wrap tr:last-child td{border-bottom:0}
.wrap td.num,.wrap th.num{text-align:right}
.wrap tr.link{cursor:pointer} .wrap tr.link:hover td{background:var(--grid)}
.rid{font-family:"IBM Plex Mono",monospace;font-size:11px;color:var(--muted)}
.seg{display:inline-flex;border:1px solid var(--rule);border-radius:6px;overflow:hidden;background:var(--surface)}
.seg button{font:inherit;font-size:13px;padding:5px 14px;border:0;background:transparent;color:var(--ink-2);cursor:pointer}
.seg button[aria-pressed="true"]{background:var(--ink);color:var(--bg)}
.seg button:focus-visible{outline:2px solid var(--speed);outline-offset:-2px}
.levels{display:grid;grid-template-columns:repeat(auto-fit,minmax(290px,1fr));gap:12px}
.level{background:var(--surface);border:1px solid var(--rule);border-radius:8px;padding:14px 16px;display:flex;flex-direction:column;gap:10px}
...
ol.top li{display:grid;grid-template-columns:1fr auto;gap:10px;align-items:baseline}
...
.chart text{fill:var(--ink-2);font-family:"IBM Plex Sans",sans-serif;font-size:12px}
...
#tip{position:fixed;pointer-events:none;background:var(--ink);color:var(--bg);font-size:12px;padding:6px 9px;border-radius:5px;max-width:320px;line-height:1.4;z-index:5}
ul.plain{margin:0;padding-left:18px;color:var(--ink-2);font-size:13px;display:flex;flex-direction:column;gap:4px;max-width:80ch}
</style></head><body>
__HEADER__
<div class="wrap">
  <header class="lede">
    <span class="eyebrow">Imputation runs · overview</span>
    <h1>What the published runs say</h1>
    <p>These runs are the ones published on this site, all computed from one snapshot of the review table so they're directly comparable. They cover headline estimates at three aggregation levels, one-knob sensitivity checks against the occupation-level headline, and leave-one-out validation of rank recovery. Full tables are on <a href="runs.html">Runs</a>.</p>
...
}).catch(() => { document.querySelector('.wrap').innerHTML = '<p>No run summary published. Build the site with a run manifest (scripts/generate_site_runs.py).</p>'; });
</script>
</body></html>
"""
...
def main():
    ...
    pages = [
        ...
        ("summary.html",     _SUMMARY_HTML,     {"__STYLE_COMMON__": _STYLE_COMMON, "__HEADER__": _header("summary")}),
        ("runs.html",        _RUNS_HTML,        {"__STYLE__": _RUNS_STYLE,   "__HEADER__": _header("runs")}),
        ("loo.html",         _LOO_HTML,         {"__STYLE__": _LOO_STYLE,    "__HEADER__": _header("runs")}),
